...
func DefaultPubKeys(loadDotSSH bool) ([]PubKey, error) {
	...
	if runtime.GOOS == "windows" {
		privPath := filepath.Join(configDir, filenames.UserPrivateKey)
		pubPath := filepath.Join(configDir, filenames.UserPublicKey)
		privPathUnix, err := toUnixKeyPath(privPath)
		if err != nil {
			return nil, err
		}
		pubPathUnix, err := toUnixKeyPath(pubPath)
		if err != nil {
			return nil, err
		}
		privPathCanon := ioutilx.CanonicalWindowsPath(privPath)
		pubPathCanon := ioutilx.CanonicalWindowsPath(pubPath)
		logrus.Info("Syncing default keys with host")
		_, err = call([]string{"sync_lima_file", path.Dir(privPathUnix), privPathCanon, privPathUnix, "600"})
		if err != nil {
			return nil, err
		}
		_, err = call([]string{"sync_lima_file", path.Dir(pubPathUnix), pubPathCanon, pubPathUnix, "644"})
		if err != nil {
			return nil, err
		}
	}
...
func CommonOpts(sshPath string, useDotSSH bool) ([]string, error) {
	...
	var opts []string
	if runtime.GOOS == "windows" {
		unixKeyPath, err := toUnixKeyPath(privateKeyPath)
		if err != nil {
			return nil, err
		}
		opts = []string{fmt.Sprintf(`IdentityFile='%s'`, unixKeyPath)}
	} else {
...
func toUnixKeyPath(keyPath string) (string, error) {
	userDir, err := call([]string{"realpath", "~"})
	if err != nil {
		return "", err
	}
	...
	relPath, err := filepath.Rel(limaDir, keyPath)
	if err != nil {
		return "", err
	}
	return path.Join(userDir, ".lima", filepath.ToSlash(relPath)), nil
}
...
func SSHOpts(sshPath, instDir, username string, useDotSSH, forwardAgent, forwardX11, forwardX11Trusted bool) ([]string, error) {
	...
	if runtime.GOOS == "windows" {
		...
		controlPath = fmt.Sprintf(`ControlPath='%s'`, path.Join("/run/user/1000", strings.TrimPrefix(controlSock, "_mnt_")))
	}
	opts = append(opts,
